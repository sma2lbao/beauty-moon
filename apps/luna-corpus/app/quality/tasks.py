...
def _run_eval_task(evaluation_id: str, judge: QualityJudge | None = None) -> None:
    ...
    Fully fail-safe: never raises into the caller (BackgroundTasks). Any
    unexpected error is logged and swallowed so it cannot surface as an
    unhandled exception in the request lifecycle.
    """
    try:
        ...
        logger.warning(
            "eval_task_unrecoverable", evaluation_id=evaluation_id, exc_info=True
        )
